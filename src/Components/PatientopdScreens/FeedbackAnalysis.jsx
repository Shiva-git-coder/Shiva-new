...
function FeedbackAnalysis() {
    ...
    const [currentIncompletePage, setCurrentIncompletePage] = useState(Number(searchParams.get("incompletePage")) || 1);
    const [currentCompletePage, setCurrentCompletePage] = useState(Number(searchParams.get("completePage")) || 1);
    const [searchTermIncomplete, setSearchTermIncomplete] = useState(searchParams.get("incompleteSearch") || "");
    const [searchTermCompleted, setSearchTermCompleted] = useState(searchParams.get("completeSearch") || "");
    ...
    const CompletedPatients = (id) => {
        setLoading(true);
        axios.get(`${resources.APPLICATION_URL}eligibleForFeedbackAnalysis`, {
            params: {
                receptionistId: id
            }
        }).then(response => {
    ...
    const handleSubmit = (index, id) => {
        ...
        setLoading(true);
        axios.post(`${resources.APPLICATION_URL}savePatientFeedbackAnalysis`, newRow).then(response => {
    ...
    const totalIncompletePages = Math.ceil(incompletePatients.length / itemsPerPage);
    const incompleteCurrentItems = incompletePatients.slice(
        (currentIncompletePage - 1) * itemsPerPage,
        currentIncompletePage * itemsPerPage
    );

    // Pagination for completed patients
    const totalCompletePages = Math.ceil(completedPatients.length / itemsPerPage);
    const completeCurrentItems = completedPatients.slice(
        (currentCompletePage - 1) * itemsPerPage,
        currentCompletePage * itemsPerPage
    );

    // Handlers for incomplete tab
    const handleIncompletePageChange = (newPage) => {
        setCurrentIncompletePage(newPage);
        setSearchParams({
            ...Object.fromEntries(searchParams),
            incompletePage: newPage,
        });
    };

    const handleIncompleteSearchChange = (event) => {
        const newSearch = event.target.value;
        setSearchTermIncomplete(newSearch);
        setSearchParams({
            ...Object.fromEntries(searchParams),
            incompleteSearch: newSearch,
            incompletePage: 1, // Reset to first page
        });
    };

    // Handlers for completed tab
    const handleCompletePageChange = (newPage) => {
        setCurrentCompletePage(newPage);
        setSearchParams({
            ...Object.fromEntries(searchParams),
            completePage: newPage,
        });
    };

    const handleCompleteSearchChange = (event) => {
        const newSearch = event.target.value;
        setSearchTermCompleted(newSearch);
        setSearchParams({
            ...Object.fromEntries(searchParams),
            completeSearch: newSearch,
            completePage: 1, // Reset to first page
        });
    };
    return (
        <div className='remainder-container'>
            <h2 className='page_title'>Feedback Analysis</h2>
            <Box className='remainder-tabcontainer' >
                {currentTab === 'remainder-today' &&
                    <input
                        ...
                        onChange={handleIncompleteSearchChange}
                ...
                {currentTab === 'remainder-complete' &&
                    <input
                        ...
                        onChange={handleCompleteSearchChange}
                        placeholder="Search by patientId, patientName, or mobile (Completed)"
                    />}
                <Box>
                    <Tabs
                        value={currentTab}
                ...
                <div>
                                ...
                                <React.Fragment>
                                    {incompletePatients.length === 0 ? (<tbody>
                                        ...
                                        <tbody>
                                            ...
                                            {incompleteCurrentItems.map((item, index) => (
                    ...
                    {incompletePatients.length > 0 && (
                        <div className="pagination-controls">
                            <button
                                onClick={() => handleIncompletePageChange(currentIncompletePage - 1)}
                            ...
                            <span  className="pagination-info">
                                Page {currentIncompletePage} of {totalIncompletePages}
                            </span>
                            <button
                                onClick={() => handleIncompletePageChange(currentIncompletePage + 1)}
                                disabled={currentIncompletePage === totalIncompletePages}
                                className="pagination-btn"
                                style={{ backgroundColor: "#00796B", color: "white" }}
                            >
                                Next
            ...
            {currentTab === 'remainder-complete' && (
                <div>
                            ...
                            {loading ? <Loader /> : (
                                <React.Fragment>
                                    {completedPatients.length === 0 ? (<tbody>
                                        ...
                                        <tbody>
                                            {completeCurrentItems.map((item, index) => (
                                                <tr key={item.patientId}>
                                                    <td>{index + 1}</td>
                                                    <td onClick={() => view(item.patientId)}>{item.patientId}</td>
                                                    <td onClick={() => view(item.patientId)}>{item.patientName}</td>
                    ...
                    {completedPatients.length > 0 && (
                        <div className="pagination-controls">
                            <button
                                onClick={() => handleCompletePageChange(currentCompletePage - 1)}
                        ...
                        style={{ backgroundColor: "#00796B", color: "white" }}
                            ...
                            <span className="pagination-info">
                                Page {currentCompletePage} of {totalCompletePages}
                            </span>
                            <button
                                onClick={() => handleCompletePageChange(currentCompletePage + 1)}
